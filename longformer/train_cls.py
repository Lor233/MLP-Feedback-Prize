...
from utils_cls import EarlyStopping, prepare_training_data, evaluate, target_id_map
...
if __name__ == "__main__":
    ...
    df_NER = pd.read_csv('../bigbird/NER_token/train_NER.csv')
    df_NER.entities = df_NER.entities.apply(lambda x: literal_eval(x) )

    train_df = df[df["kfold"] != args.fold].reset_index(drop=True)
    valid_df = df[df["kfold"] == args.fold].reset_index(drop=True)

    tokenizer = AutoTokenizer.from_pretrained(args.model)
    training_samples = prepare_training_data('train', train_df, tokenizer, args, num_jobs=NUM_JOBS)
